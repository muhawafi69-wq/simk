...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 30, 2018 at 01:49 PM
...
CREATE TABLE IF NOT EXISTS `tbl_diagnosa_penyakit` (
...
CREATE TABLE IF NOT EXISTS `tbl_jadwal_praktek_dokter` (
...
CREATE TABLE IF NOT EXISTS `tbl_kesehatan_ibu_anak` (
...
CREATE TABLE IF NOT EXISTS `tbl_laboratorium` (
...
CREATE TABLE IF NOT EXISTS `tbl_operasi` (
...
CREATE TABLE IF NOT EXISTS `tbl_paramedis` (
...
CREATE TABLE IF NOT EXISTS `tbl_penanganan_operasi` (
...
CREATE TABLE IF NOT EXISTS `tbl_pengadaan_obat` (
...
--
DELIMITER //
CREATE TRIGGER `pengadaan_obat` AFTER INSERT ON `tbl_pengadaan_obat`
 FOR EACH ROW BEGIN
...
CREATE TABLE IF NOT EXISTS `tbl_pengeluaran_obat` (
...
--
DELIMITER //
CREATE TRIGGER `pengeluaran_obat` AFTER INSERT ON `tbl_pengeluaran_obat`
 FOR EACH ROW BEGIN
...
CREATE TABLE IF NOT EXISTS `tbl_perbaikan_gizi` (
...
--
DELIMITER //
CREATE TRIGGER `perbaikan_gizi` AFTER INSERT ON `tbl_perbaikan_gizi`
 FOR EACH ROW BEGIN
UPDATE tbl_stok_obat
SET jumlah = jumlah - new.jumlah
...
CREATE TABLE IF NOT EXISTS `tbl_resep_obat` (
...
CREATE TABLE IF NOT EXISTS `tbl_riwayat_tindakan` (
...
CREATE TABLE IF NOT EXISTS `tbl_stok_obat` (
  `kode_obat` varchar(5) NOT NULL,
...
ALTER TABLE `tbl_paramedis`
 ADD PRIMARY KEY (`kode_paramedis`), ADD KEY `id_spesialis` (`id_poli`);
...
ALTER TABLE `tbl_pegawai`
 ADD PRIMARY KEY (`id_pegawai`), ADD KEY `id_bidang` (`id_bidang`), ADD KEY `id_jabatan` (`id_jabatan`);
